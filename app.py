    ...
    if columna_candidata:
        ...
        if st.button("Iniciar Cotejo de Marcas", type="primary"):
            ...
            with tab_juego:
                ...
                # Incrustamos el minijuego de HTML externo
            ...
            with tab_progreso:
                for p in range(1, total_paginas + 1):
                    porcentaje = int((p / total_paginas) * 100)
                    progress_bar.progress(porcentaje)
                    status_text.text(f"Analizando página {p} de {total_paginas} ({porcentaje}%)")
                ...
                if hallazgos:
                    df_res = pd.DataFrame(hallazgos).drop_duplicates()
                    st.balloons()
                    st.success(f"Se encontraron {len(df_res)} coincidencias de marcas.")
                    ...
                    st.warning("No se encontraron coincidencias directas con las marcas de tu portafolio en esta gaceta.")
    else:
